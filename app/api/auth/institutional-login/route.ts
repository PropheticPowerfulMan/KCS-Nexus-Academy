...
export async function POST(request: NextRequest) {
  ...
  const login = await fetch(nexusApi.replace(/\/$/, "") + "/api/auth/login", { method: "POST", cache: "no-store", headers: { "content-type": "application/json", "x-kcs-local-auth-only": "true" }, body: JSON.stringify({ identifier, password }) });
