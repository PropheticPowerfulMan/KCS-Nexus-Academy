...
export type AcademyIdentity = { userId: string; orbitId: string; organizationId: string; role: string; displayName?: string; email?: string; expiresAt: string };
...
function validateLocalSession(token: string): AcademyIdentity | null {
  ...
  try { const payload = JSON.parse(Buffer.from(parts[1], "base64url").toString("utf8")) as AcademyIdentity & { exp: number };
    ...
    return { userId: payload.userId, orbitId: payload.orbitId, organizationId: payload.organizationId, role: payload.role, displayName: payload.displayName, email: payload.email, expiresAt: payload.expiresAt };
  } catch { return null; }
}
...
export function createLocalAcademySession(account: LocalAccount) {
  ...
  const identity: AcademyIdentity = { userId: account.userId || account.email, orbitId: account.orbitId || account.email, organizationId: account.organizationId || process.env.ACADEMY_ORGANIZATION_ID || "kcs", role: account.role, email: account.email, expiresAt: expiresAt.toISOString() };
...
export type AcademyCourse = { id:string; name:string; code:string; description:string; grade:string; credits:number; updatedAt:string; enrolledStudents?:number; teacherName?:string; schedules:Array<{id:string;day:string;startTime:string;endTime:string;room:string}>; assignments:Array<{id:string;title:string;description:string;dueDate:string;maxScore:number;type:string}> };
export type AcademyScheduleEntry = { id:string; sourceKey:string; academicYear:string; semester:number; grade?:string; classGroup?:string|null; day:string; period:number; startTime:string; endTime:string; room:string; teacher:string; sourceDocument:string; course:{id:string;name:string;code:string;description:string} };
export type AcademyContext = { source:string; displayName:string; courses?:AcademyCourse[]; dailySchedule?:AcademyScheduleEntry[]; profile?:{id:string;studentNumber:string;grade:string;section:string}|null; population?:{students:number;parents:number;teachers:number;courses:number} };
export async function getAcademyContext(identity: AcademyIdentity): Promise<AcademyContext | null> { const nexusUrl=process.env.NEXUS_API_URL; const key=process.env.ACADEMY_INTEGRATION_KEY; if(!nexusUrl||!key||!identity.email)return null; try { const response=await fetch(nexusUrl.replace(/\/$/,"")+"/api/academy/context",{method:"POST",cache:"no-store",headers:{"content-type":"application/json","x-api-key":key},body:JSON.stringify({email:identity.email,role:identity.role})}); if(!response.ok)return null; const payload=await response.json() as {data?:AcademyContext}; return payload.data??null } catch { return null } }